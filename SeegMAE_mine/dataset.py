...
class AudiosetDataset(Dataset):
    def __init__(self, dataset_json_file, audio_conf, label_csv=None, use_fbank=False, fbank_dir=None,
        ...
        print('using following mask: {:d} freq, {:d} time'.format(self.audio_conf.get('freqm'),
...
class SeegDataset(Dataset):
    ...
    def __init__(self, args):
        self.data_path_folder = args.data_path_folder
        print('##### start read data path folder #####')
        self.samples_path = getFilelist(self.data_path_folder, ext='pkl')
        print('##### end read data path folder #####')
        self.samples_path = self.samples_path[:args.files_num]
        # random.shuffle(self.samples_path)

        self.samples_mean = binary2dict(args.data_path_mean_std)['mean']
        self.samples_std = binary2dict(args.data_path_mean_std)['std']
    ...
    def __getitem__(self, index):
        ...
        sample_path = self.samples_path[index]
        sample = binary2dict(sample_path)
        sample = sample['sample']
        sample = (sample - self.samples_mean) / self.samples_std
