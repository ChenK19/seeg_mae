...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=1,
                 ...
                 use_custom_patch=True, split_pos=False, pos_trainable=False, use_nce=False, beta=4.0, decoder_mode=0,
                 mask_t_prob=0.6, mask_f_prob=0.5, mask_2d=False,
                 epoch=0, no_shift=False, sample_freq=400, is_freq=True
                 ):
        super().__init__()

        self.is_freq = is_freq
        # self.spec = Spectrogram(n_fft=400, power=2, hop_length=4)
        self.spec = Spectrogram(n_fft=400, power=2, hop_length=10)
    ...
    def forward_encoder(self, x, mask_ratio, mask_2d=False):
        ...
        if mask_2d:
            x_mask, mask, ids_restore = self.random_masking_2d(x, mask_t_prob=self.mask_t_prob, mask_f_prob=self.mask_f_prob)
        else:
            x_mask, mask, ids_restore = self.random_masking(x, mask_ratio)
        ...
        return x, mask, ids_restore, x_mask
    ...
    def forward_loss(self, x, pred, mask, norm_pix_loss=False):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
    ...
    def forward(self, x, mask_ratio=0.8):
        ...
        # print(x.shape)
        emb_enc, mask, ids_restore, x_mask = self.forward_encoder(x, mask_ratio, mask_2d=self.mask_2d)
        ...
        if not self.is_freq:
            input_specs = self.spec(x)
        else:
            input_specs = x


        # pred_has_nan = torch.isnan(pred).any()
        # x_has_nan = torch.isnan(x).any()
        #
        # print('x has nan: {}, pred has nan: {}'.format(x_has_nan, pred_has_nan))

        loss_contrastive = torch.FloatTensor([0.0]).cuda()
        return loss_recon, pred, mask, loss_contrastive, input_specs
